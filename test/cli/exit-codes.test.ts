...
beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), "grimoire-index-exit-"));
...
describe("usage errors exit 64", () => {
  it("unknown flag", async () => {
    expect(await run(["node", "grimoire-index", "init", dir, "--quick", "--nope"])).toBe(64);
  });

  it("unknown subcommand", async () => {
    expect(await run(["node", "grimoire-index", "frobnicate"])).toBe(64);
  });

  it("flag value outside its choices", async () => {
    expect(
      await run(["node", "grimoire-index", "init", dir, "--quick", "--forge", "bitbucket"]),
    ).toBe(64);
  });

  it("build --out-dir that would delete the index root", async () => {
    expect(await run(["node", "grimoire-index", "build", dir, "--out-dir", "."])).toBe(64);
  });

  it("validate with no forge to detect", async () => {
    vi.stubEnv("GITHUB_ACTIONS", "");
    vi.stubEnv("GITLAB_CI", "");
    expect(await run(["node", "grimoire-index", "validate", "--root", dir])).toBe(64);
  });
  ...
  it("validate with no author to determine", async () => {
    ...
    expect(
      await run(["node", "grimoire-index", "validate", "--root", dir, "--forge", "github"]),
    ).toBe(64);
  });
});
...
describe("bad input data exits 65", () => {
  it("init --base-url that is not a URL", async () => {
    expect(
      await run(["node", "grimoire-index", "init", dir, "--quick", "--base-url", "not-a-url"]),
    ).toBe(65);
    expect(fs.existsSync(path.join(dir, "grimoire-index.config.json"))).toBe(false);
  });

  it("init --name that is not a valid identifier", async () => {
    expect(
      await run(["node", "grimoire-index", "init", dir, "--quick", "--name", "Not A Name"]),
    ).toBe(65);
  });
...
describe("success exits 0", () => {
  it("--help", async () => {
    expect(await run(["node", "grimoire-index", "--help"])).toBe(0);
  });

  it("--version", async () => {
    expect(await run(["node", "grimoire-index", "--version"])).toBe(0);
  });
});
