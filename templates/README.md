...
| `grimoire-index.config.json` | Site identity: title, base URL, registry, branding |
...
CI runs `grimoire-index validate` on it: a clean run means the entry is
...
```sh
npx grimoire-index build      # index/** -> dist/
npx grimoire-index validate   # run the contribution gate locally
```

## Upgrading CI

The workflows in this repo are thin callers of reusable workflows that
[grimoire-index](https://github.com/grimoire-rs/grimoire-index) owns. To
